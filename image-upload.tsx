...
export function ApplyForm({ jobId }: { jobId: number }) {
  ...
  const loginHref = `/login?next=${encodeURIComponent(pathname || "/oglasi")}`;
  ...
  if (state === "guest") return (
    <div className="empty">
      ...
      <div className="actions">
        <Link className="btn blue sm" href={loginHref}>Prijava</Link>
        <Link className="btn ghost sm" href="/registracija?intent=job_seeker">Registracija</Link>
      </div>
    </div>
  );
