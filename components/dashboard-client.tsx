...
export function DashboardClient({ expectedRole, title }: { expectedRole: Exclude<UserRole, "guest">; title: string }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (role !== expectedRole && role !== "admin") {
        window.location.href = role === "company" ? "/firma" : role === "admin" ? "/admin" : "/profil";
        return;
      }
      const applications = expectedRole === "candidate" ? await supabase.from("job_applications").select("*,jobs(title,companies(name))").eq("candidate_id", user.id).order("created_at", { ascending: false }) : { data: [] };
  ...
  if (loading) return <div className="panel"><h1>Učitavanje</h1><p className="lead">Spremamo podatke.</p></div>;
  ...
  return (
    <section>
      ...
      <div className="dash-grid">
        ...
        <div className="metric"><strong>{account.role}</strong><span>Uloga</span></div>
      </div>
      {expectedRole === "candidate" ? (
        <div className="table-card">
          ...
          {!account.applications.length ? <div className="empty"><strong>Nema prijava</strong><p>Prijave će se prikazati ovdje čim apliciraš.</p><Link className="btn blue" href="/oglasi">Otvori oglase</Link></div> : null}
        </div>
      ) : null}
    </section>
  );
