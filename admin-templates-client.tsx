...
export function LoginForm({ nextPath }: { nextPath?: string | null }) {
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    // Postavi flag PRIJE login poziva — tako RedirectIfAuthed ne može napraviti
    // race-condition redirect dok auth state stiže klijentu. Timestamp, ne trajna "1".
    setLoginRedirecting();

    const { data, error: authError } = await supabase.auth.signInWithPassword({ email, password });

    if (authError || !data.session) {
      // Login pao — očisti flag da ne ostane zaglavljen.
    ...
    clearStoredIntent();
    // Flag je odradio svoje (odluka o destinaciji je donesena) — očisti i njega.
    clearLoginRedirecting();
